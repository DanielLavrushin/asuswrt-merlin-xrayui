...
- FIXED: Guest AP on Asus routers (e.g. RT-AX56U) could not load websites when Xray was running — pings and DNS worked but TCP/443 connections hung. Traffic from guest networks was correctly redirected into Xray, but then dropped by the firmware's restrictive guest-bridge INPUT rules (which only allow DNS/DHCP/ICMP to the router). XRAYUI now whitelists its own redirected traffic in the `INPUT` chain (fwmark match for TPROXY, destination-port match for REDIRECT/DNAT) and configures `route_localnet`/`rp_filter` on guest bridges. ([#336](https://github.com/DanielLavrushin/asuswrt-merlin-xrayui/issues/336))
- FIXED: Repeated firewall reconfigures (e.g. `nat-start`, service restarts, `firewall configure` events) accumulated duplicate iptables hooks — every reload added another `INPUT → XRAYUI`, `FORWARD → XRAYUI` and `mangle:FORWARD/OUTPUT` TCPMSS-clamp rule. `configure_firewall` is now idempotent and `cleanup_firewall` sweeps up legacy duplicates from prior installs.
