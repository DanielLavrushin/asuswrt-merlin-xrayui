...
- ADDED: New **TUN routing** option in General Options, shown once you add a TUN inbound. Leave it on **Full** and XRAYUI keeps doing what it always did: it brings the interface up, gives it its addresses and sets up the routing so your LAN traffic goes through the tunnel. Switch it to **Interface only** and XRAYUI just creates the interface and stops there — Xray starts with the TUN inbound, but nothing is sent into it. Use that when another tool (such as [B4](https://docs.b4core.app/)) on your router already decides what should go into the tunnel, so the two don't fight over the same interface.
- CHANGED: The TUN inbound settings now follow Xray-core's own naming. When TUN support first arrived, Xray had no way to configure the interface at all, so XRAYUI set it up on the router itself and kept those settings under its own names. Xray has since added them, so **IP addresses** is now **Interface addresses** (`gateway`) and **Routes** is now **Auto system routes** (`autoSystemRoutingTable`), and new settings are available: **Outbound interface binding**, **DNS servers**, **Interface description** and **User level**. Only the ones your installed Xray-core version actually supports are shown. Your existing values are carried over automatically, so there is nothing you need to do.
- FIXED: **Routes** on a TUN inbound did the opposite of what its help text promised — it sent those networks around the tunnel instead of into it. As **Auto system routes** it now routes traffic into the tunnel, as described.
- REMOVED: The **Enable GSO** checkbox on TUN inbounds. Xray deliberately keeps segmentation offload off for forwarded traffic, and never offered a setting for it, so the checkbox had no effect.
- FIXED: Stopping Xray left some TUN routing rules behind on the router; they piled up and were never removed. They are now cleaned up properly.
- FIXED: If you gave your TUN interface a name other than `xray0`, stopping Xray left it up with its address still assigned. Any interface name is now cleaned up.
...
- ADDED: A new **Import mode** choice in the import dialog. Besides adding the imported connection as a new one or doing a complete setup, you can now **replace an existing connection** — handy when your VPN server changes and you want to swap it via a link or QR code while keeping all your routing rules and other settings untouched. ([#370](https://github.com/DanielLavrushin/asuswrt-merlin-xrayui/issues/370))
- CHANGED: The default browser fingerprint is now **firefox** instead of **chrome** — the chrome fingerprint currently causes stalled or hanging connections on some networks (notably in Russia since June 2026). This applies to imported Reality links that don't specify a fingerprint and to QR codes/links generated for router clients. Links that explicitly set a fingerprint are still respected — if your connection hangs with chrome, change the fingerprint to firefox in the outbound's security settings. ([#361](https://github.com/DanielLavrushin/asuswrt-merlin-xrayui/issues/361))
- ADDED: A new **Observatory probe interval** setting in General Options (visible when **Check connection** is enabled). Until now Xray probed every outbound every 10 seconds; with many outbounds (for example from subscriptions) and some of them dead or unreachable, these checks could slowly eat the router's memory until it hung. The default interval is now 30 seconds, and you can raise it further on low-memory routers.
- FIXED: With the XRAYUI panel open in the browser, connection status checks could overlap and pile up on slow or busy routers, making memory problems worse. The panel now waits for the previous check to finish before starting the next one, and a stuck check is cut off after a few seconds instead of hanging forever.
