...
<head>
  ...
  <script>
    ...
    function config_to_form() {
      ...
      form.xray_clients_add_id.value = uuid();
      let inbounds = xray.config.inbounds[0];
      let outbounds = xray.config.outbounds[0];

      if (inbounds.port) {
        let port = `${inbounds.port}`.split("-");
        form.xray_inbound_port_from.value = port[0];
        form.xray_inbound_port_to.value = port[port.length - 1];
      }

      form.xray_protocol.value = inbounds.protocol;
      form.xray_inbound_address.value = inbounds.listen || "0.0.0.0";

      if (inbounds.settings.sniffing) {
        form.xray_sniffing.value = inbounds.settings.sniffing && inbounds.settings.sniffing.enabled;
        form.xray_sniffing_meta.value = inbounds.settings.sniffing && inbounds.settings.sniffing.metadataOnly;
        if (inbounds.settings.sniffing.destOverride) {

          inbounds.settings.sniffing.destOverride.forEach((protocol) => {
            form[`xray_sniffing_do_${protocol}`].checked = true;
          });
        }
      }
      form.xray_sniffing.forEach((chck) => {
        chck.dispatchEvent(new Event('change'));
      });
      form.xray_sniffing_meta.forEach((chck) => {
        chck.dispatchEvent(new Event('change'));
      });

      form.xray_allocate.value = inbounds.allocate ? inbounds.allocate.strategy : "always";
      form.xray_allocate_refresh.value = inbounds.allocate ? inbounds.allocate.refresh : 5;
      form.xray_allocate_concurrency.value = inbounds.allocate ? inbounds.allocate.concurrency : 3;

      if (inbounds.streamSettings) {
        form.xray_network.value = inbounds.streamSettings.network || "tcp";
        form.xray_network_tcp_accept_proxy.value = inbounds.streamSettings.tcpSettings && inbounds.streamSettings.tcpSettings.acceptProxyProtocol;
        form.xray_network.dispatchEvent(new Event('change'));
      }

      form.xray_allocate.dispatchEvent(new Event('change'))
    ...
    function buildSniffing() {
      ...
      const destOverride = snifChck && !metaChck
        ? ["http", "tls", "quic", "fakedns"].filter((protocol) => form[`xray_sniffing_do_${protocol}`].checked)
        : [];

      xray.config.inbounds[0].settings.sniffing = {
        enabled: snifChck,
        destOverride: destOverride,
        metadataOnly: metaChck,
      };
    }
    ...
    function allocateChange() {
      const allocate = document.form.xray_allocate.value;
      let inbounds = xray.config.inbounds[0];
      inbounds.allocate = {
        strategy: allocate
      };
      if (allocate === "random") {
        inbounds.allocate.refresh = document.form.xray_allocate_refresh.value;
        inbounds.allocate.concurrency = document.form.xray_allocate_concurrency.value;
      }
      document.querySelectorAll("table tr.xray_alopt_row").forEach((row) => row.style.display = allocate === "random" ? "table-row" : "none");
    }

    function networkChange() {
      let network_rows = document.querySelectorAll(".xray_network_opt_row");
      network_rows.forEach((row) => row.style.display = "none");

      const network = document.form.xray_network.value;
      document.querySelectorAll(`.xray_network_opt_row_${network}`).forEach((row) => row.style.display = "table-row");
      ...
      if (network === "tcp") {
        inbounds.streamSettings.tcpSettings = {
          acceptProxyProtocol: document.form.xray_network_tcp_accept_proxy.checked,
          header: { type: "none" }
        };
      }
...
<body onload="initial();">
  ...
  <form method="post" name="form" id="ruleForm" action="/start_apply.htm" target="hidden_frame">
    ...
    <input type="hidden" name="preferred_lang" id="preferred_lang" value="<% nvram_get(" preferred_lang"); %>"> <input
      type="hidden" name="firmver" value="<% nvram_get(" firmver"); %>">
                    ...
                    <tr bgcolor="#4D595D">
                      <td valign="top">
                        <div class="formfontdesc">
                          ...
                          <div id="formfontdesc" class="formfontdesc">This UI control page provides a simple interface
                            to manage and monitor the X-ray Core's configuration and it's status.</div>
                          <div style="margin: 10px 0 10px 5px" class="splitLine"></div>
                          <table width="100%" border="1" align="center" cellpadding="2" cellspacing="0"
                            bordercolor="#6b8fa3" class="FormTable SettingsTable" style="border: 0px"
                            id="xray_table_config">
                            <thead>
                              <tr>
                            ...
                            <tr>
                              ...
                              <td>
                                ...
                                <span id="xray_server_status_links">
                                  <a id="xray_server_status_restart_link" class="button_gen button_gen_small"
                                    href="javascript:serverStatus('restart');void(0);">Restart</a>
                                  <a id="xray_server_status_stop_link" class="button_gen button_gen_small"
                                    href="javascript:serverStatus('stop');void(0);">Stop</a>
                                </span>
                              </td>
                            </tr>
                            <tr>
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'The listening address, either an IP address or a Unix domain socket. The default value is <b>0.0.0.0</b>, which means accepting connections on all network interfaces.');">The
                                  listening address</a>
                              </th>
                              <td>
                                <input type="text" maxlength="15" class="input_20_table" name="xray_inbound_address"
                                  onkeypress="return validator.isIPAddr(this, event);" autocomplete="off"
                            ...
                            <tr>
                              <th>Inbound Port</th>
                              <td>
                                <input type="text" maxlength="5" class="input_6_table" name="xray_inbound_port_from"
                                  value="" autocorrect="off" autocapitalize="off"
                                  onKeyPress="return validator.isNumber(this,event);" />
                                -
                                <input type="text" maxlength="5" class="input_6_table" name="xray_inbound_port_to"
                                  value="" autocorrect="off" autocapitalize="off"
                                  onKeyPress="return validator.isNumber(this,event);" />
                              </td>
                            </tr>
                            ...
                            <tr class="xray_alopt_row">
                              ...
                              <td>
                                <input type="text" maxlength="2" class="input_6_table" name="xray_allocate_refresh"
                                  onkeypress="return validator.isNumber(this,event);" value="" />
                            ...
                            <tr class="xray_alopt_row">
                              ...
                              <td>
                                <input type="text" maxlength="2" class="input_6_table" name="xray_allocate_concurrency"
                                  onkeypress="return validator.isNumber(this,event);" value="" />
                            ...
                            <tr>
                              ...
                              <td>
                                <input type="radio" name="xray_sniffing" id="xray_sniffing_enabled" class="input"
                                  value="true" onchange="buildSniffing()" />
                                <label for="xray_sniffing_enabled" class="settingvalue">Enabled</label>
                                <input type="radio" name="xray_sniffing" id="xray_sniffing_disabled" class="input"
                                  value="false" onchange="buildSniffing()" checked />
                            ...
                            <tr id="xray_sniffing_metadataonly">
                              ...
                              <td>
                                <input type="radio" name="xray_sniffing_meta" id="xray_sniffing_meta_enabled"
                                  class="input" value="true" onchange="buildSniffing()" />
                                <label for="xray_sniffing_meta_enabled" class="settingvalue">Enabled</label>
                                <input type="radio" name="xray_sniffing_meta" id="xray_sniffing_meta_disabled"
                                  class="input" value="false" onchange="buildSniffing()" checked />
                            ...
                            <tr id="xray_sniffing_destoverride">
                              ...
                              <td>
                                <input type="checkbox" name="xray_sniffing_do_http" id="xray_sniffing_do_http"
                                  class="input" value="http" onchange="buildSniffing()" />
                                <label for="xray_sniffing_do_http" class="settingvalue">HTTP</label>
                                <input type="checkbox" name="xray_sniffing_do_tls" id="xray_sniffing_do_tls"
                                  class="input" value="tls" onchange="buildSniffing()" />
                                <label for="xray_sniffing_do_tls" class="settingvalue">TLS</label>
                                <input type="checkbox" name="xray_sniffing_do_quic" id="xray_sniffing_do_quic"
                                  class="input" value="quic" onchange="buildSniffing()" />
                                <label for="xray_sniffing_do_quic" class="settingvalue">QUIC</label>
                                <input type="checkbox" name="xray_sniffing_do_fakedns" id="xray_sniffing_do_fakedns"
                                  class="input" value="fakedns" onchange="buildSniffing()" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_tcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Only used for inbound, indicating whether to accept the PROXY protocol.');">Accept
                                  the PROXY protocol</a>
                              </th>
                              <td>
                                <input type="checkbox" name="xray_network_tcp_accept_proxy" class="input" value="true"
                                  onchange="buildSniffing()" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Maximum transmission unit. It indicates the maxium bytes that an UDP packet can carry');">MTU</a>
                              </th>
                              <td>
                                <input type="text" maxlength="4" class="input_6_table" name="xray_network_kcp_mtu"
                                  onkeypress="return validator.isNumber(this,event);" value="1350" />
                                <span class="hint-color">Recommended value is between 576 and 1460, default is
                                  1350</span>
                              </td>
                            </tr>
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Transmission time interval, measured in milliseconds (ms), determines how often mKCP sends data');">TTI</a>
                              </th>
                              <td>
                                <input type="text" maxlength="3" class="input_6_table" name="xray_network_kcp_tti"
                                  onkeypress="return validator.isNumber(this,event);" value="50" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Uplink capacity refers to the maximum bandwidth used by the host to send data, measured in MB/s (note: Byte, not bit). It can be set to 0, indicating a very small');">Uplink
                                  Capacity</a>
                              </th>
                              <td>
                                <input type="text" maxlength="3" class="input_6_table"
                                  name="xray_network_kcp_uplinkCapacity"
                                  onkeypress="return validator.isNumber(this,event);" value="5" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Downlink capacity refers to the maximum bandwidth used by the host to receive data, measured in MB/s (note: Byte, not bit). It can be set to 0, indicating a very small bandwidth.');">Downlink
                                  Capacity</a>
                              </th>
                              <td>
                                <input type="text" maxlength="3" class="input_6_table"
                                  name="xray_network_kcp_downlinkCapacity"
                                  onkeypress="return validator.isNumber(this,event);" value="20" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'Whether or not to enable congestion control. When congestion control is enabled, Xray will detect network quality. It will send less packets when packet loss is severe, or more packets when network is not fully filled.');">Congestion</a>
                              </th>
                              <td>
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'The read buffer size for a single connection, measured in MB');">Read
                                  buffer size</a>
                              </th>
                              <td>
                                <input type="text" maxlength="3" class="input_6_table"
                                  name="xray_network_kcp_readbufsize"
                                  onkeypress="return validator.isNumber(this,event);" value="2" />
                            ...
                            <tr class="xray_network_opt_row xray_network_opt_row_kcp">
                              <th>
                                <a class="hintstyle" href="javascript:void(0);"
                                  onmouseover="hint(this,'The write buffer size for a single connection, measured in MB');">Write
                                  buffer size</a>
                              </th>
                              <td>
                                <input type="text" maxlength="3" class="input_6_table"
                                  name="xray_network_kcp_writebufsize"
                                  onkeypress="return validator.isNumber(this,event);" value="2" />
                                <span class="hint-color">default: 2</span>
                              </td>
                            </tr>
                        ...
                        <div class="formfontdesc">
                          <table width="100%" border="1" align="center" cellpadding="2" cellspacing="0"
                            bordercolor="#6b8fa3" class="FormTable SettingsTable tableApi_table" style="border: 0px"
                            id="xray_table_inbound_clients">
                            <thead>
                              <tr>
                            ...
                            <tbody id="xray_table_clients">
                              ...
                              <tr class="row_title">
                                ...
                                <td><input name="xray_clients_add_id" maxlength="36" type="text"
                                    class="input_25_table" /></td>
                                <td>
                                  <input type="button" class="add_btn" onclick="clients_add();" value="" />
                                  <a href="#" class="button_gen button_gen_small" style="visibility: hidden">Get
                                    Congig</a>
                                </td>
                              </tr>
