...
Включает постоянную проверку доступности исходящих соединений. Когда опция активна, XRAYUI запускает Xray со встроенным модулем **Observatory**, который периодически открывает соединение через **каждый** outbound к URL проверки и фиксирует результат. Статус отображается зелёным/красным индикатором рядом с каждым outbound в разделе **Outbounds**; эти же данные использует автоматический failover подписок. Системные элементы, которые XRAYUI добавляет для этого, помечены тегом `sys` и скрыты в UI.

#### URL проверки Observatory

URL, который Observatory запрашивает через каждый outbound. Эндпоинт должен возвращать **HTTP 204 (No Content)**. По умолчанию: `https://www.google.com/generate_204`.

#### Интервал проверки Observatory

Как часто (в секундах) Observatory проверяет исходящие подключения. По умолчанию — **30** секунд.

::: warning Роутеры с малым объёмом памяти
Каждая проверка открывает соединение через все outbound. При большом их количестве (например, импортированных из подписок) — особенно если часть серверов недоступна — короткий интервал заметно повышает нагрузку на процессор и память и в худшем случае может исчерпать всю оперативную память роутера. Увеличьте интервал, если в конфигурации много outbound.
:::
