<template>
  <tr>
    ...
    <td>
      <label class="go-option"> <input type="checkbox" v-model="startup" @click="updatestartup" /> Start Xray on startup</label>
      <label class="go-option" v-show="validateCheckConOption()"> <input type="checkbox" v-model="checkconenabled" @change="setcheckconnection" /> Check connection to xray server</label>
      <modal ref="connModal" title="Enable Connection Check" width="450">
        <div class="formfontdesc">
          ...
          <p>XRAYUI will insert a system rule into the routing rules and create a SOCKS inbound proxy for this check. These system settings are tagged with `sys` and will remain hidden in the UI.</p>
        </div>
        <template #footer>
