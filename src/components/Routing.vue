      ...
      <tr>
        <td colspan="2">
          ...
          <hint>
            ...
            <p>A common use case is to split domestic and foreign traffic. Xray can use its internal mechanisms to determine the traffic from different regions and then send them to different outbound proxies.</p>
          </hint>
        </td>
      </tr>
    </thead>
    <tbody>
            ...
            <ul>
              ...
              <li>
                **IPIfNonMatch**: If the domain name does not match any rule, resolve the domain name into an IP address (A record or AAAA record) and match it again
                <ul>
                  <li>When a domain name has multiple A records, it will try to match all A records until one of them matches a rule</li>
                  <li>The resolved IP only works for routing selection, and the original domain name is still used in the forwarded packets</li>
                </ul>
              </li>
              <li>**IPOnDemand**: If any IP-based rules are encountered during matching, immediately resolve the domain name into an IP address for matching</li>
            </ul>
          </hint>
        </th>
        <td>
      ...
      <tr v-if="routing.rules">
        <th>
          Rules
          <hint>
            An array corresponding to a list of rules. For each connection, the routing will judge these rules from top to bottom in order. When it encounters the first effective rule, it will forward the connection to the outboundTag or balancerTag specified by the rule.
      ...
      <tr v-if="routing.rules">
        <th>
          ...
          <hint> Update the GeoIP and GeoSite metadata files. This operation will take some time, please be patient. </hint>
        </th>
        <td>
          <input class="button_gen button_gen_small" type="button" value="manage local files" @click.prevent="manage_geodat()" />
          <geodat-modal ref="geodatModal"></geodat-modal>
          <input class="button_gen button_gen_small" type="button" value="update community files" @click.prevent="update_geodat()" />
      ...
      <tr v-if="engine.mode == 'client' && routing.portsPolicy">
        <th>
          ...
          <hint>
            By default, the mode `redirect` is used, meaning that traffic on all ports is redirected to the inbound port of xray. Specify any additional ports that should be routed through or bypass Xray.
            <ul>
              <li><strong>redirect</strong>: Traffic on all ports is redirected to the inbound port. You define the ports that should NOT be redirected to Xray.</li>
              <li><strong>bypass</strong>: Traffic on all ports bypasses Xray. You define the ports that should be explicitly redirected to Xray.</li>
            </ul>
          </hint>
        </th>
        <td>
          mode: {{ routing.portsPolicy.mode }}
          <input class="button_gen button_gen_small" type="button" value="manage" @click.prevent="manage_redirect_ports()" />
