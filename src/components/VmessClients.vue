...
export default defineComponent({
  ...
  methods: {},
  props: {
    clients: Array<XrayInboundClientObject>,
  },
  setup(props,) {
    let uuid = () => {
    ...
    const clients = ref<XrayInboundClientObject[]>(props.clients ?? []);
    ...
    const addClient = async () => {
      ...
      if (newClient.value.email && newClient.value.id) {
        await engine.submit(SubmtActions.clientAdd, newClient.value, 1000);
        newClient.value.email = "";
        resetNewForm();
    ...
    const removeClient = async (client: XrayInboundClientObject) => {
      window.showLoading();
      await engine.submit(SubmtActions.clientDelete, client, 1000);
      await engine.loadXrayConfig();
      window.hideLoading();
    };
