<template>
  <modal ref="modal" title="Show XRAY configuration file" width="800px">
    ...
    <template v-slot:footer>
      <label><input type="checkbox" v-model="hideSenseData" @change="hide_sense_data()" /> hide sensitive data</label>
      <input class="button_gen button_gen_small" type="button" value="copy to clipboard" @click.prevent="copy_to_clipboard()" />
      <a class="button_gen button_gen_small" :href="configUri" target="_blank">open raw</a>
    </template>
  </modal>
</template>
<script lang="ts">
        ...
        try {
          ...
          if (!successful) {
            ...
            if (hideSenseData.value) {
              alert("Configuration copied to clipboard. Sensitive data has been hidden. However, please review the configuration before sharing.");
            } else {
              alert("Configuration copied to clipboard. Warning: Sensitive data is visible. Please be cautious when sharing.");
            }
          }
        } catch (err) {
