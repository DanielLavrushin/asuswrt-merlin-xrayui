        ...
        <tr v-for="(r, index) in rules.filter((r) => !r.isSystem())" :key="index">
          ...
          <td>
            ...
            <span class="row-buttons">
              <input class="button_gen button_gen_small" type="button" value="&#8593;" @click="reorderRule(r)"
                v-if="index > 0" />
    ...
    <table class="FormTable modal-form-table">
      <tbody>
        ...
        <tr>
          <th>
            Outbound Connection
            <hint> The tag of the outbound proxy. This rule will take effect when the outbound connection matches the
              tag.
            </hint>
          </th>
          <td>
        ...
        <tr>
          <th>
            Inbound Connection
            <hint> An array where each item represents an inbound proxy tag. This rule will take effect when the inbound
              connection matches any of the tags in the array. </hint>
          </th>
          <td>
        ...
        <tr>
          <th>
            Apply to users
            <hint>
              Apply this rule to specific users. This rule will take effect when the source user matches any of the
              email.
            </hint>
          </th>
        ...
        <tr>
          <th>
            Network
            <hint> This can be **tcp**, **udp**, or **tcp,udp**. This rule will take effect when the connection method
              is
              the specified one. </hint>
          </th>
          <td>
        ...
        <tr>
          <th>
            ...
            <hint>
              ...
              current
              connection matches any of the protocols in the array.
        ...
        <tr>
          <th>
            Target IP List
            <hint> An array where each item represents an IP range. This rule will take effect when the target IP
              matches
              any of the IP ranges in the array. </hint>
          </th>
          <td>
            <div class="textarea-wrapper">
        ...
        <tr>
          <th>
            Source IP List
            <hint> An array where each item represents an IP range in the format of IP, CIDR, GeoIP, or loading IP from
              a
              file. This rule will take effect when the source IP matches any of the IP ranges in the array. </hint>
          </th>
          <td>
            <div class="textarea-wrapper">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const show = () => {
      ...
      inbounds.value = xrayConfig.inbounds.filter((o) => !o.isSystem()).map((o) => o.tag).filter((tag): tag is string => tag !== undefined);
      outbounds.value = xrayConfig.outbounds.filter((o) => !o.isSystem()).map((o) => o.tag).filter((tag): tag is string => tag !== undefined);
