<template>
    <modal ref="modal" title="Transports specify how Xray communicates with peers">
        <div class="formfontdesc">
            <p> Transports specify how to achieve stable data transmission. Both ends of a connection often need to
                specify the same transport protocol to successfully establish a connection. Like, if one end uses
                WebSocket, the other end must also use WebSocket, or else the connection cannot be established.</p>
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="input_option" v-model="transport.security">
                                <option v-for="(opt, index) in securityOptions" :key="index" :value="opt">{{ opt }}
                                </option>
                            </select>
                            <span class="row-buttons" v-if="transport.security != 'none'">
                                <input class="button_gen button_gen_small" type="button" value="settings"
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="input_option" v-model="transport.network">
                                <option v-for="(opt, index) in transportOptions" :key="index" :value="opt">{{ opt }}
                                </option>
                            </select>
