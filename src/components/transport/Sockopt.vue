            ...
            <tbody>
                    ...
                    <th>
                        ...
                        <hint>
                            ...
                            <ul>
                                ...
                                <li>redirect: Use the transparent proxy in Redirect mode. It supports all TCP
                                    connections
                                    based on `IPv4/6`.</li>
                                <li>tproxy: Use the transparent proxy in TProxy mode. It supports all TCP and UDP
                                    connections
                                    based on `IPv4/6`.</li>
                            </ul>
                ...
                <tr>
                    <th>
                        Domain strategy
                        <hint>
                            When the target address is a domain name, the corresponding value is configured, and the
                            behavior of SystemDialer is as follows:
                            <ul>
                                <li>`AsIs`: Resolve the IP address using the system DNS server and connect to the domain
                                    name.</li>
                                <li>"UseIP", "UseIPv4", and "UseIPv6": Resolve the IP address using the built-in DNS
                                    server and connect to the IP address directly.</li>
                            </ul>
                        </hint>
                    </th>
                    <td>
                        <select class="input_option" v-model="sockopt.domainStrategy">
                            <option v-for="(opt, index) in domainStrategyOptions" :key="index" :value="opt">{{ opt }}
                            </option>
                        </select>
                ...
                <tr>
                    <th>
                        Mark
                        <hint>
                            The mark value of the transparent proxy. The mark value is used to mark the packets that
                            need
                            to be processed by the transparent proxy.

                            An `integer` value. When its value is non-zero, `SO_MARK` is marked with this value on the
                            outbound connection.
                            <ul>
