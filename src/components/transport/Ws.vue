<template>
  <tbody v-if="transport.wsSettings">
    ...
    <tr>
      <th>The HTTP path
        <hint>
          ...
          <br />
          If path contains the ed query parameter, early data will be activated for latency reduction, and its value
          will be the length threshold of the first packet. If the length of the first packet exceeds this value, early
          data won't be activated. The recommended value is `2560`, with a maximum of `8192`. Compatibility problems can
          occur when the value is set too high. Try lowering the threshold when encountering such problems.
        </hint>
      </th>
      <td>
    ...
    <tr>
      <th>The Host header
        <hint>
          The `Host` header sent in HTTP requests. Defaults to an empty string. Servers will not validate the `Host`
          header
          sent by clients when left blank.
          <br />
          If the `Host` header has been defined on the server in any way, the server will validate if the `Host` header
          matches.
          <br />
